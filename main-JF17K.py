...
class Experiment:
    def __init__(self, num_iterations, batch_size, lr, dr, dembd, dembd1, max_ary, layer_num, ary_list):
    ...
    def train_and_eval(self):
        ...
        model = HMTE(len(d.ent2id), len(d.rel2id), self.dembd, self.dembd1, self.max_ary, self.device, self.layer_num, ent_relnel=d.ent_relnel, ary_list = self.ary_list)
        model.to(device)
                ...
                with torch.no_grad():
                    ...
                    if it - best_valid_iter >= param['valid_patience'] or it == self.num_iterations:
                        ...
                        for i, ary in enumerate(args.ary_list):
                            ...
                            # log.write('Testing Arity:%d, MRR=%f, Hits@10=%f, Hits@3=%f, Hits@1=%f' % (
                            # ary, best_valid_metric['group_test_mrr'][i], best_valid_metric['group_test_hits'][i][2],
                            # best_valid_metric['group_test_hits'][i][1], best_valid_metric['group_test_hits'][i][0]))
                        print(args)
                        ...
                        # log.write('Best epoch %d' % best_valid_iter)
                        # log.write('Hits @10: {0}'.format(best_valid_metric['test_hit10']))
                        # log.write('Hits @3: {0}'.format(best_valid_metric['test_hit3']))
                        # log.write('Hits @1: {0}'.format(best_valid_metric['test_hit1']))
                        # log.write('Mean reciprocal rank: {0}'.format(best_valid_metric['test_mrr']))
                        # print(mrr_lst)
...
def save_checkpoint(state, filename='checkpoint/JF17K/checkpoint.tar'):
    torch.save(state, filename)
    shutil.copyfile(filename, 'checkpoint/JF17K/model_best.tar')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dembed1", type=int, default=64, nargs="?")
    ...
    Exp = Experiment(num_iterations=param['num_iterations'], batch_size=param['batch_size'], lr=param['lr'], dr=param['dr'],
                     dembd=param['dembed'], dembd1=param['dembed1'], max_ary=d.max_ary, layer_num = param['layer_num'], ary_list = args.ary_list)
