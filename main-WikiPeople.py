...
class Experiment:
    ...
    def train_and_eval(self):
        ...
        model = HMTE(len(d.ent2id), len(d.rel2id), self.dembd, self.dembd1, self.max_ary, self.device, self.layer_num, d.ent_relnel, ary_list = self.ary_list)
        model.to(device)
...
def save_checkpoint(state, filename='checkpoint/WikiPeople_wT/checkpoint.tar'):
    torch.save(state, filename)
    shutil.copyfile(filename, 'checkpoint/WikiPeople_wT/model_best.tar')
